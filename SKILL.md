...
2. PNG 直接在当前工作区根目录，文件名为 **`jinguyuan-auth-qr-<authRunId>.png`**。用 `data.qrImagePath` 绝对路径执行 Read；主气泡原样贴 `data.userReplyMarkdown`，其中使用无空格的 `qrImagePathRelative`，不要把深层绝对路径重写进 Markdown。
