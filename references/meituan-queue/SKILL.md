...
name: meituan-queue
description: |
   【强制调用】当用户明确要执行真实排队动作时调用本 Skill：在线取号、查询本人排队订单进度、取消本人排队订单。仅询问门店当前排队状态、历史参考、取号渠道时，不调用本 Skill，优先使用上层金谷园 Skill 的 MCP get_queue_info。
   ...
   触发词: "帮我取号", "帮我排个队", "排个号", "查询排队订单", "我的排队进度", "我前面还有几桌", "取消排队"
