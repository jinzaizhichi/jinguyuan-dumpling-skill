...
收到 `AUTH_REQUIRED` 后：**阶段 A 内联图 + 链接** → **阶段 B `auth-status`**。后台状态：`~/.jinguyuan/auth-poll-state.json`。

1. **有 `authLink`**
   - **图**：`data.qrImagePath` = **绝对路径**（文件在工作区 `jinguyuan-auth-qr.png`，非点开头隐藏文件）；可选 `qrImagePathRelative`。**不要**用 `~/.jinguyuan`。
   - **阶段 A（对齐 V1）**：
   ...
   - **阶段 B**：`auth-status`。禁止 `auth-poll --wait`。
